...
| 产物 | 状态 |
|---|---|
| `src-ui/vtouch_ui.cpp`（1985 行，ImGui 面板） | ✅ 已从备份还原 |
...
**数据流**：核心写 `g.phys[]/g.virt[]` + 区域表 → 面板直接读（零拷贝）；面板写区域表 + 面板矩形 + `stop_req` → 核心下一轮自然读到（区域线程 1ms 一轮，`vt_region.c:185-188`）。
...
**为什么 `g` 能整体搬进映射**（已验证）：全库只有一处定义（`src/vtouchd.c:54`）；结构体纯 POD、**无指针成员**（可整块 `memcpy`）；没有别的标识符叫 `g` → 用
...
| `src/vt_region_table.h`（新） | 区 B 的布局 + 自旋锁 + `gen` 双检封装 | ~60 行 | 双侧契约唯一出处 |
...
面板代码里那 11 个 `extern "C"` 声明（`src-ui/vtouch_ui.cpp:28-47`）**一行不改**，`ui_glue.c` 换实现：
...
| `vtouch_set_hooks()` | **按值拷贝**（`vtouch_ui.cpp:1924-1925` 传的是栈上临时量；存指针会悬空 → 曾导致 SIGBUS）。`consume` 语义改成"面板往区 B 推矩形，核心自判" |

## 8. 旋转 / 横屏适配（本次重点）

### 8.1 结论：核心**完全不需要**旋转感知

核心/脚本/区域表/事件坐标统一是**竖屏逻辑坐标**（`-w/-h` 指定，`vtouchd.c:73-77`）：

| 环节 | 位置 | 说明 |
|---|---|---|
| raw → 逻辑 | `vt_util.c:65`（`raw_to_logical`），`size = axis ? logical_height : logical_width` | 区域判定用 |
| 逻辑 → raw | `vt_util.c:43`（`logical_to_raw`） | 注入用（WS 命令 `vt_ws.c:498-536`） |
| 区域合法性 | `vt_region.c:45` | 以 `logical_width/height` 为界 |
...
`src-ui/vtouch_ui.cpp:186-201` 的模型：
...
| 3 | **ImGui 画布** | `vtouch_ui.cpp:1601` | `io.DisplaySize = 当前屏尺寸`；旋转后强制全量重绘（`g_force_frames`） |
| 4 | **ImGui 输入** | `:1613-1614` `AddMousePosEvent(mx,my)` | 触点先 `p2c` 再喂 ImGui → 面板交互在横屏同样准确 |
| 5 | **命中判定** | `:381 in_panel()` | 一律在内空间算；谓词 `ui_consume_cb` 先把外部坐标 `p2c` 进来（`:401`） |
| 6 | **面板自身落位** | `:204-220 on_display()` | 未拖动过则回右上角（公式与 `nativeInit` 一致）；拖过则夹回屏内 |
| 7 | **几何唯一来源** | `:73` 注释 | 面板几何有**三处同公式**（`#define` + `panel_w()`/`in_panel()` + `build_panel()`）→ 改一处必须三处同改，否则"看着对、点不着" |
| 8 | **稳定窗** | `:191`、`:210`、`:400` | 换向后 500ms 内吞触摸谓词**一律返回 0**（宁放不吞）。原因见 `:208-209` 注释：谓词是"按下问一次、锁存整段手势"，一次错判会吞掉/漏掉整段 |
| 9 | **区域绘制/轨迹** | `:611/814/862/886/914/1610` | 区域表与触点是**外空间**数据 → 画之前统一 `p2c` |
| 10 | **面板矩形推给核心**（新架构新增） | `vt_glue.c` 新代码 | 必须推**竖屏逻辑坐标**（内→外的逆变换）→ 这是搬迁后唯一的新增旋转适配点，且是**静默 bug 型**（写错只是吞错位置，不报错） |
| 11 | **稳定窗推给核心**（新架构新增） | 同上 | 核心看不到 Java 的稳定窗 → 窗内**推空矩形**（等同"不吞"），把策略 8 搬到新架构 |

### 8.4 旋转策略定型：C（重建 surface）

四条策略真机判定完毕（详见 git 历史中的 `probe/README.md`；探针已按要求从仓库移除）：
...
| 风险 | 对策 |
|---|---|
| 共享内存 bug 难查 | `magic`+`ver` 校验；`--dump-shm` 调试开关；把区布局收在一个头文件（`vt_region_table.h`） |
...
| 3 | 被吞的触点要不要也从区域事件里排除 | 排除（+3 行；现状 `region_match()` 不查 `phys_eaten`，被吞的手仍会触发脚本区域事件） |
