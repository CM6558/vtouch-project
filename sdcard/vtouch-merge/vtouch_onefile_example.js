 ...
 *                         滑动，durationMs 默认 300ms，内部线程按帧插值，不阻塞主线程
 ...
 *   client.gesture(frames[, durationMs])  连续提交多帧；第二参为可选总时长，
 *                         0 或缺省 = 立即逐帧提交（无内置时序）
 *   client.pinch(cx,cy,startGap,endGap,durationMs)  双指缩放，durationMs 生效
 *   client.reset()        释放全部模拟触点
 *   client.close()        关闭连接
 *   client.onError        错误回调，如 vt.onError = function(msg){ log(msg); }
 *
...
var vt = new VTouch().connect(function (client) {
    ...
    /* 样例 6：双指缩放（durationMs 生效，内部线程步进插值） */
    // client.pinch(client.width / 2, client.height / 2, 200, 1000, 500);

    /* 样例 7：手势帧序列（可选第二参总时长，0/缺省 = 立即逐帧提交） */
    // client.gesture([
    //     [
