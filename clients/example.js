...
/* ---------- 3.5 开关型区域（r1）：按一下开、再按一下关 ---------- */
/* 面板里画一个矩形、id 取 r1（卡片上能改名）。按它一次 = 开，再按一次 = 关。
 * 开着时它在屏幕上会变成「半透明绿底 + 粗绿边 + 名字居中 + ●开」——
 * 样式不用脚本管：toggle 翻转时会通知核心，核心→面板同步（面板按需重绘，收到就立刻补一帧）。
 * 状态存在脚本侧，所以**任何回调、任何线程**里都能读、能改。 */
var r1 = vt.toggle("r1", {
    on: false,                                     // 初始状态（默认 false）
    toast: true,                                   // 每次翻转弹一句（调试友好，正式用可去掉）
    onChange: function (on) { log("开关 r1 → " + (on ? "开" : "关")); }
});
/* 手动控制（例：别的回调里想关掉它 —— 关掉时屏幕上的高亮同步消失）：
 *   r1.on            读当前状态（布尔）
 *   r1.value()       同上
 *   r1.set(false)    强制关掉
 *   r1.flip()        翻转
 *   r1.stop()        不再监听 r1 的按压（当前状态保留）
 * 注：直接调 vt.mark("r1", 0) 只改面板样式、不动 r1.on —— 要两边一起变就用 set/flip。 */

/* ---------- 3.6 完整按压 → 开关开着才执行，执行完自动关掉开关 ---------- */
...
var press = vt.onRegionPress(REGION_ID, function (g) {
    if (!r1.on) { log("按压 " + REGION_ID + "，但开关 r1 没开 → 跳过"); return; }   // ← 回调里判断开关
    log("完整按压（开关已开）：slot" + g.slot + " " + g.down.x + "," + g.down.y
        + " → " + g.up.x + "," + g.up.y + "，历时 " + g.ms + " ms");
    sleep(2300);                                   // 慢活直接写（回调本来就在独立线程里）
    vt.finger().tap(151, 2251);                    // ← 换成你要点的坐标
    vt.finger().tap(149, 2001);
    r1.set(false);                                 // ← 执行完一次就关掉（面板高亮同步消失）
});
// press.stop();                                   // 不要了就停（还没抬起的按压也一起收掉）
