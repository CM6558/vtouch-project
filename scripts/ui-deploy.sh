...
#   sh scripts/ui-deploy.sh build     # 只编：UI 核心 + 面板（real 模式）—— 不需要 adb
#   sh scripts/ui-deploy.sh deploy    # 只推：核心 + 设备侧起停脚本（2 个文件）+ md5 对账
...
#
# 步骤编号统一 [1/5]..[5/5]：编面板 → 编核心 → 推 → 起 → 核对自解包。
#
...
do_deploy() {
    ...
    echo "[3/5] 推核心 + 设备侧起停脚本（面板三件套在核心里面，不用单独推）"
...
case "${1:-all}" in
  build)  do_build ;;                     # 只编不推：不需要 adb（以前这里无谓地 need_adb）
  deploy) need_adb; do_deploy ;;
  start)  need_adb; echo "[4/5] 起（核心会自己拉起面板）"; ondev start ;;
  stop)   need_adb; ondev stop ;;
  status) need_adb; ondev status ;;
  all)    need_adb; do_build; do_deploy; echo "[4/5] 起（核心会自己拉起面板）"; ondev start; do_verify_extracted ;;
