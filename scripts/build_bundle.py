...
CORE = '''/* ============================================================================
 * VTouch 单文件包：内嵌 ImGui 面板 dex/so（headless 构建才内嵌 vtouchd）+ 一层薄函数。
 * 手机上只要这一个文件。首次运行自动释放面板二进制（需 root，一次），之后直启。
...
var CURR = null;
function vtouchCur() {
    if (!CURR) throw new Error("无当前连接：用 vt.onRegion(...)（自带连接），或先 vt.uiStart() + vt.connect()");
    return CURR;
}
...
function vtouchInstall() {
    ...
    if (!VTOUCH_BIN_SIZE) throw new Error("这个 bundle 没内嵌 vtouchd（面板构建）：headless 兜底不可用 —— 用面板，或换 headless 包");
...
module.exports = {
    ...
    reset: vtouchReset,
    stop: vtouchStop,
    uiShow: function () { return vtouchUiCmd(1); },   /* 叫回被「关闭 UI」关掉的面板 */
    uiHide: function () { return vtouchUiCmd(0); },   /* 屏幕零占用（进程/事件照跑） */
...
/* ---- 面板屏幕占用：「关闭 UI」的恢复通道 --------------------------------------
 * vt.uiShow() / vt.uiHide()：在当前连接上发一条 `ui show|hide`（面板必须有连接；
 * 没有连接就抛错，提示用 onRegion 或自己 connect）。面板进程照跑——关掉只是
 * 屏幕零占用（面板 + overlay 都不画，图层藏掉不参与合成），EVIOCGRAB / WS /
 * 区域表 / 事件分发全不受影响：脚本该收的 region_ev 一条不少。
 * onRegion 建连后会自动发一次 `ui show`，所以「关掉了面板，再跑一次脚本」就能叫回来。 */
function vtouchUiCmd(show) {
    var c = CURR || (LISTEN && LISTEN.conn);
    if (!c) throw new Error("ui " + (show ? "show" : "hide")
        + "：当前没有连接 —— 用 vt.onRegion(...)（会自动叫回面板），或先 uiStart+connect");
    vtouchSend(c, "ui " + (show ? "show" : "hide"));
    return true;
}
...
function vtouchOnRegion(a, b, c) {
    ...
    if (!LISTEN) {
        ...
        vtouchSub(conn, "region");
        vtouchSend(conn, "ui show");     /* 上一轮可能点过「关闭 UI」→ 起脚本就把它叫回来 */
       /* 只订区域事件：不再白收高频 pev（C） */
...
def main() -> int:
    ...
    if headless:
        if not BIN.exists():
            print("缺 %s — headless bundle 要它：先编核心（README 第 ① 步）" % BIN)
            return 1
        raw = BIN.read_bytes()
        _b64js = _b64_lines(base64.b64encode(raw).decode("ascii"))   # 分行：单行 30KB base64 会被内网 WAF 拦
        blob = ("/* ---- 内嵌 vtouchd（headless 构建才有） ---- */\n"
                "var VTOUCH_BIN_SIZE = %d;\n"
                'var VTOUCH_BIN_B64 = "%s";\n' % (len(raw), _b64js))
    else:
        # 面板构建：核心在面板 .so 里，不需要 vtouchd（省 58KB）；headless 兜底会明确报错
        blob = ("/* ---- 面板构建：不内嵌 vtouchd ---- */\n"
                "var VTOUCH_BIN_SIZE = 0;\n"
                'var VTOUCH_BIN_B64 = "";\n')
    uiblob = ""
    if headless:
