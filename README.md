...
| `sub [region]` | `ok` / `err sub` | 裸 `sub` = 订阅区域通道（只有这一条推送） |
...
| 推送 | 形状 | 什么时候来 |
|---|---|---|
| `region_ev` | `region_ev <id> <ev> <slot> <x> <y>` | 区域事件，`ev` ∈ `down/enter/move/exit/up`（订阅之后才有） |

只有这一条推送，而且它**只由物理手指产生**：虚拟触点压根不进转发队列（§4.4）——
这就是「回触不会自己触发自己」的保证。
...
1. **转发（§4.1）**：物理帧边界（每个 `SYN_REPORT`）在 `emit_frame()` 之后做一次状态比较，
   ...
   只有物理手指入队；虚拟触点的变化**不发**（区域判定只吃真手指，源头就不自激）。
...
5. **出站（§4.5）**：响应 / `region_ev` 全部进**出站队列**，主循环 `poll` 里只有一个刷出点
...
6. **订阅与踢（§4.6）**：订阅位决定推不推（只有区域事件这一路）；断连/被踢时清订阅位并丢弃残包（不会串给下一个客户端）。
