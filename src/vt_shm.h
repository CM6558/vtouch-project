 ...
 *   · 一个 memfd，三段映射。**准确性声明**（评审 §3 对账过）：面板 attach 时先把整段按 RW 映射一次
 *     （头部与区 B 必须可写），再**另映射**一个区 A 的只读视图并只走它 —— 所以「面板改不了状态」
 *     目前靠的是**默认路径纪律**，不是 MMU 强制（那个整段 RW 映射并没有 munmap 掉）。
 *     要真强制就得 `munmap(base)` 或只映射各自需要的段。
 ...
 *     核心自然吃掉并按既有 region_add/region_del/region_rename 语义生效 —— 有唤醒 pipe（正常路径）
 *     时是**即时**的（~1ms），没有它才退到 ≤8ms 的一轮。
 *
